...
{{- if .Values.global.clusterDomain }}
    {{- if .Values.postgresql.useOperator -}}
        {{.Release.Name}}-db.{{.Release.Namespace}}.svc.{{.Values.global.clusterDomain}}
    {{- else -}}
        {{.Release.Name}}-postgresql.{{.Release.Namespace}}.svc.{{.Values.global.clusterDomain}}
    {{- end -}}
{{- else -}}
    {{- if .Values.postgresql.useOperator -}}
        {{.Release.Name}}-db.{{.Release.Namespace}}.svc.kubernetes.{{.Values.global.region}}.{{.Values.global.tld}}
    {{- else -}}
        {{.Release.Name}}-postgresql.{{.Release.Namespace}}.svc.kubernetes.{{.Values.global.region}}.{{.Values.global.tld}}
    {{- end -}}
{{- end -}}
{{ end }}
